...
Profiles may select Anthropic explicit-cache duration with `anthropicCacheTtl: '5m' | '1h'` (default `'5m'`). The pinned Python serializer emits fixed ephemeral markers without a duration; selecting one hour is a deliberate target profile option. Provider adapters apply the selected duration uniformly to existing request breakpoints, including lifted tool-result markers, without changing persisted messages, cache selection or opt-out behavior. Five-minute requests retain the upstream wire shape without `ttl`. Keep this separate from OpenAI `promptCacheRetention`. Review upstream caching and profile changes against this choice; see [Anthropic cache evidence](../transpile/anthropic-cache.md).
